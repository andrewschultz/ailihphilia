...
check requesting the score:
	...
	if player has epicer recipe and north tron is off-stage:
		say "You also have [tron-got] of [number of tronparts] piece[if tron-got is not 1]s[end if] of the North-Tron, according to the epicer recipe.";
...
DNA hand	soot tattoos	"The DNA hand doesn't need augmentation, and besids, the soot tattoos are sort of generic. Maybe there's a way to make a pattern of them."
...
rep popper	scorn rocs	"The scorn rocs scorn such abstract concepts as 'rep' and, hence, are not affected. Perhaps there is a more productive way to vandalize them."
...
roto motor	eroded ore	"The roto motor probably runs on some sort of battery. It seems charged, anyway."
...
this is the Sniffins-successful rule:
...
to say trigirt:
	if row-prog > 1:
		say "The tri-girl makes perfect sense now that you've changed Worn Row to Word Row and Work Row";
...
Dirge Grid is a room in Dim Mid. "The only way back is south[if Diktat Kid is in Dirge Grid], but you can't really run away from the Diktat Kid. A Tru Hurt is pointed at you, and that redivider looks extra imposing, but if you deal with the Kid, that won't matter[else], and you might as well go that way, now you've vanquished the Diktat Kid. Saner Arenas surround the XILE helix that stands where the redivider used to be[end if]."
...
Diktat Kid is a proper-named person in Dirge Grid. description is "The Diktat Kid [if henchmen-left is 0]is yelling at you, now[else if henchmen-left is 1]alternates between yelling at the departed [hl of true] and the remaining [hl of false][else]is barking out orders needlessly to the Knife Fink and Verses Rev[end if]. Even though it doesn't do any good, you feel spine nips worrying the Kid has a gun snug."
...
every turn when Yuge Guy is quicknear:
	say "The Yuge Guy booms '[next-rand-txt of table of yuge battle cries][run paragraph on]'[paragraph break]";
...
instead of showing to:
	if show-warn is true, say "SHOW x TO y is the equivalent of GIVE x TO y, which maps to USE x ON y.";
	now show-warn is true;
	try useoning noun with second noun;

givesubbing it to is an action applying to two things.

Understand "give [someone] [something preferably held]" as givesubbing it to (with nouns reversed).
...
instead of givesubbing:
	if debug-state is true, say "using givesubbing: [noun] [second noun].";
	if player has ti:
...
The elope pole is a thing. description is "It looks foldable, maybe even separable. You wonder what wild places it could help you visit."
